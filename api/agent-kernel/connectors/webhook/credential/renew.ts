...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
    ...
    try {
        ...
        const summary = await service.renewCredentialLease({
            session_id: readEnterpriseSessionId(req),
            increment_seconds: Number(req.body?.increment_seconds || 0) || undefined,
        });
        res.status(200).json({
            success: true,
        ...
        const message = error instanceof Error ? error.message : 'credential_renew_failed';
