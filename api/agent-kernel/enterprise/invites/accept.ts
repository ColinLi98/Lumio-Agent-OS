...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
    ...
    try {
        ...
        const result = await getEnterpriseAccountService().acceptInvite({
            sessionId: String(req.body?.session_id || readBearerToken(req) || '').trim(),
            inviteToken: String(req.body?.invite_token || '').trim(),
        });
        res.status(200).json({ success: true, result });
    } catch (error) {
        const message = error instanceof Error ? error.message : 'enterprise_invite_accept_failed';
